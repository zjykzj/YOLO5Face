...
from models.yolo5face.general import non_max_suppression
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            txt_path = str(save_dir / 'labels' / f"{pict_folder[p.name]}" / p.stem) + (
                '' if dataset.mode == 'image' else f'_{frame}')  # im.txt
            # Create
            os.makedirs(os.path.dirname(txt_path), exist_ok=True)

            # Write results
            # assert save_txt is True and save_conf is True
            if save_txt:
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in det:  # NOTE: for widerface_evaluate
                    if save_txt:  # Write to file
                        ...
                        new_xywh = (*xyxy[:2], *xywh[2:])
                        line = (*new_xywh, conf) if save_conf else (cls, *new_xywh)  # label format
...
def parse_opt():
    ...
    parser.add_argument('--folder_pict', default='../datasets/widerface/wider_face_split/wider_face_split.txt',
